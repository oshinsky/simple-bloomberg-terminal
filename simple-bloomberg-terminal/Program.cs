...
foreach (var c in europeCompanies)
    Console.WriteLine($"   {c.Name,-25} ${c.Revenue:F0}B");

var earningsEvents = events
    .Where(e => e.Type == EventType.EARNINGS)
    .OrderBy(e => e.Date)
    .Select(e => new { e.Title, e.Date, e.ImpactScore });

Console.WriteLine("\nEarnings događaji:");
foreach (var e in earningsEvents)
    Console.WriteLine($"   [{e.Date}] {e.Title,-35} impact: {e.ImpactScore:F1}");

var companiesPerSector = companies
    .GroupBy(c => c.Sector)
    .Select(g => new { Sector = g.Key, Count = g.Count() })
    .OrderByDescending(x => x.Count);

Console.WriteLine("\nBroj kompanija po sektoru:");
foreach (var s in companiesPerSector)
    Console.WriteLine($"   {s.Sector,-30} {s.Count} kompanija");
...
foreach (var s in avgMarginBySector)
    Console.WriteLine($"   {s.Sector,-30} {s.AvgMargin:P1}");

var avgRevenueByRegion = companies
